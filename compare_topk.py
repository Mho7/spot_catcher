...
이미지 전체 이상 점수를 산출할 때
가장 이상한 상위 k개 패치의 평균을 사용.

top-1   → 가장 이상한 패치 1개 (기본값, max)
top-3   → 상위 3개 평균
top-5   → 상위 5개 평균
top-10  → 상위 10개 평균

실행:  python compare_topk.py
"""
import os
import sys
import io
...
# 공통 모델 1개 사용 (pooling 없음, 순수 top-k 효과만 비교)
...
for k in TOPK_VALUES:
    good_score, _ = model.predict(transform(good_pil), topk=k)
    bad_score,  _ = model.predict(transform(bad_pil),  topk=k)
...
head = f"  {'top-k':<{C1}} {'정상 score':>{C2}} {'결함 score':>{C3}} {'식별도':>{C4}}"
div  = f"  {'─'*C1}  {'─'*C2}  {'─'*C3}  {'─'*C4}"

print()
print("  top-k 비교  (pooling 없음 고정)")
print(f"  {'─' * (C1+C2+C3+C4+6)}")
print(head)
print(div)
for res in results:
    marker = "  ◀" if res["k"] == best["k"] else ""
    print(f"  {'top-'+str(res['k']):<{C1}} {res['good']:>{C2}.4f}  {res['bad']:>{C3}.4f}  {res['disc']:>{C4}.4f}{marker}")
print(f"  {'─' * (C1+C2+C3+C4+6)}")
print(f"  최적: top-{best['k']}  (식별도 {best['disc']:.4f})")
print()
