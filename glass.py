...
class GLASS(torch.nn.Module):
    ...
    def trainer(self, training_data, val_data, name):
        ...
        if len(ckpt_path) != 0 and not os.path.exists(ckpt_path_save):
            LOGGER.info("Training already completed, skipping to test.")
            return 0., 0., 0., 0., 0., -1.

        def update_state_dict(epoch=0, best_record=None):
            ...
            state_dict["best_record"] = best_record
            state_dict["dsc_opt"] = self.dsc_opt.state_dict()
            if self.pre_proj > 0:
                state_dict["proj_opt"] = self.proj_opt.state_dict()
        ...
        if os.path.exists(ckpt_path_save):
            ckpt = torch.load(ckpt_path_save, map_location=self.device, weights_only=False)
            if 'discriminator' in ckpt:
                self.discriminator.load_state_dict(ckpt['discriminator'])
            if 'pre_projection' in ckpt and self.pre_proj > 0:
                self.pre_projection.load_state_dict(ckpt['pre_projection'])
            if 'dsc_opt' in ckpt:
                self.dsc_opt.load_state_dict(ckpt['dsc_opt'])
            if 'proj_opt' in ckpt and self.pre_proj > 0:
                self.proj_opt.load_state_dict(ckpt['proj_opt'])
            if 'epoch' in ckpt:
                start_epoch = ckpt['epoch'] + 1
            if 'best_record' in ckpt:
                best_record = ckpt['best_record']
            # Restore best checkpoint path
            existing_best = glob.glob(self.ckpt_dir + '/ckpt_best*')
            if len(existing_best) != 0:
                ckpt_path_best = existing_best[0]
            LOGGER.info("Resumed from epoch {}/{}".format(start_epoch, self.meta_epochs))

        pbar = tqdm.tqdm(range(start_epoch, self.meta_epochs), unit='epoch', disable=True)
        for i_epoch in pbar:
            self.forward_modules.eval()
            ...
            update_state_dict(epoch=i_epoch, best_record=best_record)
        ...
        # Training complete — remove ckpt.pth so next run skips to test
        if os.path.exists(ckpt_path_save):
    ...
    def _train_discriminator(self, input_data, cur_epoch, pbar, pbar_str1):
        ...
        for i_iter, data_item in enumerate(input_data):
            ...
            img = img.to(torch.float).to(self.device)
            if self.pre_proj > 0:
                fake_embed, patch_shapes = self._embed(aug, evaluation=False)
                ...
                fake_embed, patch_shapes = self._embed(aug, evaluation=False)
                fake_feats = fake_embed
                fake_feats.requires_grad = True
                true_feats = self._embed(img, evaluation=False)[0]
                true_feats.requires_grad = True

            mask_s = data_item["mask_s"].to(self.device).float()
            ref_h, ref_w = patch_shapes[0]
            if mask_s.shape[1] != ref_h or mask_s.shape[2] != ref_w:
                mask_s = F.interpolate(mask_s.unsqueeze(1), size=(ref_h, ref_w), mode='nearest').squeeze(1)
    ...
    def predict(self, test_dataloader):
        ...
        with tqdm.tqdm(test_dataloader, desc="Inferring...", leave=False, unit='batch', disable=True) as data_iterator:
