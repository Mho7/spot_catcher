...
def generate_thr(img_shape, min=0, max=4):
    ...
    perlin_noise_np = scipy_rotate(perlin_noise_np, angle, reshape=False, order=1, mode='constant', cval=0)
...
def perlin_mask(img_shape, feat_size, min, max, mask_fg, flag=0):
    if isinstance(feat_size, (tuple, list)):
        feat_h, feat_w = feat_size
    else:
        feat_h = feat_w = feat_size
    mask = np.zeros((feat_h, feat_w))
    while np.max(mask) == 0:
        ...
        down_ratio_y = int(img_shape[1] / feat_h)
        down_ratio_x = int(img_shape[2] / feat_w)
        mask_ = perlin_thr_fg
...
def rand_perlin_2d_np(shape, res, fade=lambda t: 6 * t ** 5 - 15 * t ** 4 + 10 * t ** 3):
    # Pad shape to be divisible by res to avoid broadcast errors
    orig_shape = shape
    shape = (math.ceil(shape[0] / res[0]) * res[0], math.ceil(shape[1] / res[1]) * res[1])
    ...
    result = math.sqrt(2) * lerp_np(lerp_np(n00, n10, t[..., 0]), lerp_np(n01, n11, t[..., 0]), t[..., 1])
    return result[:orig_shape[0], :orig_shape[1]]
